        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response) {
            ...
            foreach (var argument in arguments)
            {
                if (ElevatorLockdown.Instance.Elevators.Contains(argument.ToLower()) && ElevatorLockdown.Instance.disabledElevators.Contains(ElevatorToType(argument.ToLower())))
                {
                    ...
                    response = $"{argument.ToLower()} Elevator has been enabled by Administrator!";
                    return true;
                }
                else
                {
                    response = $"{argument.ToLower()} Elevator was already enabled by Automatic Failure System";
                    return false;
                }
            }
